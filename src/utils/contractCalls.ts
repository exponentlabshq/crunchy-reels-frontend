...
export async function getInvestorHoldings(filmId: number, investor: string) {
  const result = await callReadOnlyFunction(
    ...
    CINEBLOCK_ADDRESS,
    [`0x${serializeArg(uintCV(filmId))}`, `0x${serializeArg(principalCV(investor))}`]
  );
  return parseReadOnlyResult(result);
}
...
export async function previewInvestment(filmId: number, usdcxAmount: number) {
  const result = await callReadOnlyFunction(
    ...
    "preview-investment",
    CINEBLOCK_ADDRESS,
    [`0x${serializeArg(uintCV(filmId))}`, `0x${serializeArg(uintCV(usdcxAmount))}`]
  );
  return parseReadOnlyResult(result);
}

// ============================================
// CINEBLOCK - WRITE CALLS
// ============================================
...
export function investInFilm(filmId: number, amount: number): Promise<string> {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      functionArgs: [uintCV(filmId), uintCV(amount)],
      network,
...
export function transferToken(tokenId: number, sender: string, recipient: string): Promise<string> {
  return new Promise((resolve, reject) => {
    const options = {
      contractAddress: CINEBLOCK_ADDRESS,
      contractName: CINEBLOCK_NAME,
...
export function transferUsdcx(amount: number, sender: string, recipient: string): Promise<string> {
  return new Promise((resolve, reject) => {
    const postCondition: FungiblePostCondition = {
      type: "ft-postcondition",
      address: sender,
